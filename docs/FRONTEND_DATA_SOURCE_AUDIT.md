...
## Conclusion
All frontend components are wired directly to SQLite-backed FastAPI REST endpoints. There are **zero hardcoded fallback arrays** in production runtime paths. When the database is empty, clean onboarding empty states are displayed; when a dataset is imported, all components dynamically update to reflect the new data.
